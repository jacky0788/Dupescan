...
| 方法/屬性 | 說明 |
|-----------|------|
| `__init__(group, index)` | 建立卡片，渲染檔案清單 table |
...
| `_start_scan()` | 驗證路徑 → kill 舊 thread → 建立 worker/thread → 開始掃描 |
...
| `_on_thread_finished()` | thread.finished signal 接收者，最終恢復 idle 狀態 |
| `_on_progress(...)` | 更新步驟標籤、ETA、進度條 |
| `_on_done(groups)` | 渲染結果卡片，記錄 log |
